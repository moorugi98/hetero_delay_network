...
def plot_raster(
    filename, spike_times, spike_senders, layer, num_to_plot=100, plot_time=(6000, 8000)
):
    """
    ...
    fig, (a0, a1) = plt.subplots(
        nrows=2, ncols=1, figsize=(8, 5), gridspec_kw={"height_ratios": [3, 1]}
    )
    mask_time = (spike_times <= plot_time[1]) & (
        spike_times >= plot_time[0]
    )  # choose time to plot
    ...
    a0.scatter(
        spike_times[mask_time & mask_ids],
        spike_senders[mask_time & mask_ids],
        s=0.1,
        c="r",
    )
...
for runindex in range(runnum):
    for mod_i in range(module_depth):
        ...
        if mod_i == 2:
            if len(sys.argv) > 7:
                file = os.getcwd() + "/figs/rasterplot/{}_intra={}{}_inter={}{}_skip_double={}_d={}_w={}.pdf".format(
                    network_mode,
                    delay_mode_intra,
                    delay_intra_param,
                    delay_mode_inter,
                ...
                file = os.getcwd() + "/figs/rasterplot/{}_intra={}{}_inter={}{}.pdf".format(
                    network_mode,
                    delay_mode_intra,
                    delay_intra_param,
                    delay_mode_inter,
        ...
        measures[runindex, mod_i, :] = [
            pairwise_corr(spike_times=times, spike_senders=gids, record_time=sim_time),
...
out_arr = np.column_stack(
    (np.tile(np.arange(n), m), measures.reshape(m * n, -1))
)  # reshaping array to dataframe
...
if len(sys.argv) > 7:
    df_measure.to_csv(
        "measures_{}_intra={}{}_inter={}{}_skip_double={}_d={}_w={}.csv".format(
            network_mode,
            delay_mode_intra,
            delay_intra_param,
            delay_mode_inter,
    ...
    df_measure.to_csv(
        "measures_{}_intra={}{}_inter={}{}.csv".format(
            network_mode,
            delay_mode_intra,
            delay_intra_param,
            delay_mode_inter,
