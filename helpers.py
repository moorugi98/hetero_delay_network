...
def pairwise_corr(spike_times, spike_senders, record_time):
    ...
    spike_times = (
        np.array(spike_times) - t_onset
    )  # control for t_onset ruining time bin
    spike_senders = np.array(spike_senders)
    pairs = random.sample(
        list(itertools.combinations(np.unique(spike_senders), 2)), num_pair
    )  # num_pair rand. pairs

    for pair in pairs:
        boolean_arr = np.zeros(
            (2, int(record_time // bin_size)), dtype=bool
        )  # init spike train
        for nid, neuron in enumerate(pair):  # iterate over two neurons in each pair
            indices = np.where(neuron == np.array(spike_senders))[
                0
            ]  # indices of spike time of a current neuron
            st = (
                spike_times[indices] - 0.00001
            )  # [0.9999, 18.999, 238.9999...] # dirty trick to make binning easier
            boolean_arr[
                nid, np.int_(st // bin_size)
            ] = True  # now the array is full with binned spike train
        sum_pearson_coef += np.corrcoef(boolean_arr)[
            0, 1
        ]  # compute sum of Pearson corr. coef.
...
def revised_local_variation(spike_times, spike_senders):
    ...
    for ni, neuron in enumerate(neuron_list):
        ...
        isi = np.ediff1d(
            np.sort(np.array(spike_times)[indices])
        )  # inter spike interval
        if isi.shape[0] < 2:
            lvr[ni] = np.nan
        else:
            lvr[ni] = (3 / (isi.shape[0] - 1)) * np.sum(
...
def fano_factor(spike_times, record_time, N):
    ...
    normed = (hist / N) * (
        1000 / bin_size
    )  # num of spike of a single neuron per one sec
...
def train(volt_values, target_output, split_ratio=0.2):
    ...
    for mod_i in range(module_depth):
        ...
        fit_model = lm.RidgeClassifierCV(
            alphas=deltas, fit_intercept=True, store_cv_values=True
        ...
        deltaindex = np.where(deltas == fit_model.alpha_)[
            0
        ]  # pick delta which is actually chosen
        MSE[mod_i] = np.mean(
            fit_model.cv_values_[:, :, deltaindex], axis=(0, 1, 2)
        )  # average over all samples & feats.
    return scores, MSE
...
def reshape_arr(flat):
    return np.reshape(
        flat, (-1, module_depth, N_E)
    )  # reshape the array in (time,module,neurons)
...
def plot_V_m(filename, times, voltages, num_to_plot=5):
    ...
    selected = np.random.choice(
        np.arange(0, N), num_to_plot
    )  # for each module select num_to_plot neurons
